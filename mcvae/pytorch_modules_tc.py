# -*- coding: utf-8 -*-
"""
Created on Fri Apr 16 11:43:32 2021
...
from networks.VAE_net import encoder_cmr, decoder_cmr, encoder_tagging, decoder_tagging
...
class MultiChannelBase(torch.nn.Module):
	...
	def init_encoder(self):
		...
		for ch in self.n_feats:
			...
			if ch == 'tagging':
				print('\n' + ch + ' encoder loaded')
				W_mu.append(torch.nn.DataParallel(encoder_tagging(self.n_feats[ch][0], self.n_feats[ch][1], self.lat_dim)))
				W_logvar.append(torch.nn.DataParallel(encoder_tagging(self.n_feats[ch][0], self.n_feats[ch][1], self.lat_dim)))
			elif ch == 'cmr':
				print('\n' + ch + ' encoder loaded')
	...
	def init_decoder(self):
		...
		for ch in self.n_feats:
			...
			if ch == 'tagging':
				# print('\n' + ch + ' decoder loaded')
				W_out.append(torch.nn.DataParallel(decoder_tagging(self.n_feats[ch][0], self.n_feats[ch][1], self.lat_dim)))
	...
	def sample_from(self, qzx):
		...
		for ch in range(self.n_channels):
			if self.training:
				zx.append(
					qzx[ch].rsample()#IndexError: list index out of range
				)
			else:
				zx.append(
					qzx[ch].loc
		...
		for epoch in range(self.epochs[-2], self.epochs[-1]):
			if type(data) is torch.utils.data.DataLoader:
				...
				for gen_batch in data:
					local_batch = []
					for idx in range(len(gen_batch)-1): # All data except metadata and img_names
